...
function readConfig(ctx) {
  ...
  const required = (key) => {
    ...
    if (typeof v !== "string" || v.length === 0) {
      throw new Error(
        `mulearn-github: missing required config "${key}". Check config.yaml and the corresponding env var is set.`
      );
    }
    return v;
  };
  return {
    ...
    githubInstallationId: typeof c.githubInstallationId === "string" ? c.githubInstallationId : void 0,
    backfillSince: typeof c.backfillSince === "string" ? c.backfillSince : void 0
  };
}
var GH_API = "https://api.github.com";
var UA = "mulearn-leaderboard-plugin";
...
async function ghRest(path, jwt) {
...
async function gql(token, query, variables, logger) {
...
async function setWatermark(ctx, iso) {
  await ctx.db.execute(
...
async function ensureContributor(ctx, login, name, avatarUrl) {
...
async function upsertActivity(ctx, a) {
  await ctx.db.execute(
...
function isHuman(author) {
  return !!author && author.__typename === "User" && !!author.login && !author.login.endsWith("[bot]");
}
var REPOS_QUERY = `
...
async function listRepos(token, org, logger) {
  const repos = [];
  let cursor = null;
  do {
    const data = await gql(token, REPOS_QUERY, { org, cursor }, logger);
...
async function scrapeRepoPRs(ctx, token, org, repo, watermark) {
  let cursor = null;
  do {
    const data = await gql(token, PRS_QUERY, { org, repo, cursor }, ctx.logger);
...
async function scrapeRepoIssues(ctx, token, org, repo, watermark) {
  let cursor = null;
  do {
    const data = await gql(token, ISSUES_QUERY, { org, repo, cursor }, ctx.logger);
  ...
  async scrape(ctx) {
    ...
    for (const repo of repos) {
      try {
        ...
        ctx.logger.error(`mulearn-github: failed on repo ${repo}`, err);
      }
      done++;
